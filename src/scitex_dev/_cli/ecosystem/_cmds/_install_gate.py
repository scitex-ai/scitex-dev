...
def render_audit_gate(distribution: str, target: Path, root: Path) -> str:
    ...
    return (
        ...
        "the assertion below makes a WRONG anchor fail rather than look\n"
        "correct.\n"
        "\n"
        ...
        "\n"
        "    # A WRONG ANCHOR STILL LOOKS PINNED. `parents[N]` depends on how\n"
        "    # deep this file sits, so a copy generated for a different layout\n"
        "    # resolves confidently to the wrong directory and grades it. This\n"
        "    # assertion is the difference between `pinned` and `pinned\n"
        "    # CORRECTLY` — without it a bulk regeneration could write many\n"
        "    # plausible-looking wrong anchors and nothing would say so.\n"
        f"    anchor = Path(__file__).resolve().parents[{n}]\n"
        '    assert (anchor / "pyproject.toml").is_file(), (\n'
        ...
        "\n"
        "    # Act\n"
        "    # `path=` stays a SELF-CONTAINED expression rather than reusing\n"
        "    # `anchor`. Two reasons, and the first is not stylistic:\n"
        "    # test__install_gate_anchor.py EVALUATES this exact keyword\n"
        "    # expression with `__file__` bound, so it measures what the\n"
        "    # emitted code DOES rather than what it says. A bare `anchor`\n"
        "    # Name is not evaluable in isolation, which would have downgraded\n"
        "    # a real behavioural check into a text comparison. Second, a\n"
        "    # reader of this call sees which tree is graded without tracing\n"
        "    # a variable.\n"
