...
def _check_option_positional_ordering(
    ...
    """
    # Function-local because `_has_required_positional` now lives in
    # `._walker`. The PEP 562 `__getattr__` below forwards it for EXTERNAL
    # importers, but a module's own name lookups go through `globals()` and
    # never consult its own `__getattr__` — so an internal caller needs a
    # real import. Local rather than module-scope to keep `._walker`'s
    # import of this module cycle-free.
    from ._walker import _has_required_positional
...
# The command-tree walker. Extracted so it can be EDITED at all: this module
# is far over the repo's 512-line file cap, so the write hook refuses every
# change to it, and that had blocked four separate pieces of work on the
# walker itself. `_run.py` imports `_walk` from here, so the forwarding is
# load-bearing rather than merely historical.
_MOVED_TO_WALKER = frozenset(
    {
        "_has_required_positional",
        "_walk",
    }
)
...
def __getattr__(name: str):
    ...
    if name in _MOVED_TO_WALKER:
        from . import _walker

        return getattr(_walker, name)
