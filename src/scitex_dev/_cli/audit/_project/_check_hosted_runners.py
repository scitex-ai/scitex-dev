...
That mandate is **SUPERSEDED by the operator directive of 2026-08-05**, heard
firsthand:

    「CIのルールを緩めないといけなかったです」  — the CI rules had to be relaxed.
    「スパルタンはシーアイから外しましょう」      — take Spartan OUT of CI.
...
A gate that permits the existing mess and blocks its correction is worse than
no gate. The ratchet is gone; PS-169 is now a flat, unconditional W.
...
rule stays at E, narrowed to SELF-HOSTED label sets. A hosted label is always
served by GitHub, so it is out of PS-224's scope entirely.
"""
