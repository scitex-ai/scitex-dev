...
# ---------------------------------------------------------------------------
# Markdown rendering — for README marker injection
# ---------------------------------------------------------------------------
...
def render_markdown(payload: Dict[str, Any]) -> str:
    ...
    Output shape (designed to drop between
    ``<!-- AGENT_VERIFICATION_START -->`` / ``END`` markers):

        ## Skills Quality (verified by agent)
        > Generated by a fresh agent reading only this package's _skills/.
    ...
    Lists (when ``runs_per_prompt > 1``) are joined with ``\\n\\n---\\n\\n``.
    """
    import datetime
