...
Companion to [05_development_01_version-control.md](05_development_01_version-control.md). This skill documents the **automation commands** (CLI, MCP, Python API) that support the release workflow.
...
The dashboard reads `~/.scitex/dev/config.yaml` (or `<project>/.scitex/dev/config.yaml` if present — project overrides user; see `01_ecosystem_06_local-state-directories.md`). Project-scope config.yaml wins when both exist.
