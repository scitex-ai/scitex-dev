...
| §12  | Canonical `gui` group                        | yes (warn) | Legacy gui-adjacent leaves (`board`, `dashboard`, bare `gui`, …) flagged; a `gui` GROUP checked for `open`/`serve`/`status`/`stop`. Hidden Phase W/E aliases exempt. |
| §13  | Self-maintenance nests under `dev`           | yes (warn) | `DEV_SUBCOMMAND_NAMES` (`daemon`/`cron`/`systemd`/`hooks`/`skills`/`shell`) flagged when NOT under a `dev` ancestor. Nesting only — no fixed verb set required inside `dev`. Hidden Phase W/E aliases exempt. |
| §13a | One abstraction level per group              | yes (warn) | Flags a group whose VISIBLE direct children mix an INTENT name with a MECHANISM name from one curated axis (`daemon`/`service`/`periodic` vs `cron`/`systemd`/`timer`). One finding per parent; a mechanism name with no intent sibling is legitimate and unflagged. Curated list, NOT a classifier — see [20_dev-commands.md](20_dev-commands.md) §13a. |
