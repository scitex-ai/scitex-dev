...
17. [17_config-layout-enforcement.md](17_config-layout-enforcement.md) — **PS-222**, the mechanical rule behind 06's tracked/`runtime/` split: everything directly under `.scitex/<pkg-short>/` is TRACKED except `runtime/`; the primary config is always `config.yaml` (never a `<pkg-short>.yaml` alias); a package scope is always a DIRECTORY. Covers the three arms, the `runtime/` control arm, ignored-vs-untracked, severity `W` (the PS-220 lesson), the `audit.exemptions` opt-out with mandatory reason, and why PS-180 is a different tree
