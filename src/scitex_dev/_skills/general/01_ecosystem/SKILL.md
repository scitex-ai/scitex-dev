...
13. [13_runtime-state-db-layout.md](13_runtime-state-db-layout.md) — **The `.db` runtime-state convention is WITHDRAWN** (constitution §3, 2026-08-23: a `.db` file IS SQLite). Runtime state has ONE home, per-host PostgreSQL on 55432. What survives: `runtime/` as THE off-GPFS redirect layer for regenerable NON-database local state, a specialization of 12's `runtime_path()` (punim0264 GPFS inode-exhaustion incident, neurovista ADR-0022 — itself now describing a withdrawn convention). A `.db` under `runtime/` is a violation, not a shard
14. [14_credential-rotation-two-tier.md](14_credential-rotation-two-tier.md) — Two-tier primary-host OAuth credential rotation: one PRIMARY refresher per account, REPLICA hosts pull an access-only artifact; invariant CR-001 (exactly one refresh timer per account); `pooled` vs `exclusive:<label>` accounts; sac owns the mechanism, scitex-dev the convention
15. [15_credential-rotation-spartan-pull.md](15_credential-rotation-spartan-pull.md) — Spartan replica deliverable of 14: `sac accounts pull-token` fetches the access-only envelope via SSH-fanout, split-writes the oauth artifact + `.credentials.meta.json`; pull-at-start + re-pull-under-1h + fail-loud starvation card
