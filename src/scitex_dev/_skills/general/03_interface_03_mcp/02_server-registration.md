...
A bridge file that hand-wraps each tool with `@mcp.tool() async def <pkg>_<name>(...)` and forwards parameters by name is the **legacy** pattern. It was the cause of the 2026-04-30 `auto_stash` / `summary_only` drift incident — see [08_lessons-and-pitfalls.md](08_lessons-and-pitfalls.md). Files still using this pattern at the time of writing are tracked in [TODO.md](TODO.md).
