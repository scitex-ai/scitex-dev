...
`<cli> mcp list-tools` accepts `-v|-vv|-vvv` and `--json`, identical in shape to `list-python-apis` (see [03_interface_01_python-api/10_introspection-commands.md](../03_interface_01_python-api/10_introspection-commands.md)). Each level **adds** to the previous; never replaces.
