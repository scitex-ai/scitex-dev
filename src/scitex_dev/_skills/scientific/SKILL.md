...
- [01_figures_02_logic-and-ordering.md](01_figures_02_logic-and-ordering.md) — Universal scientific-figure LOGIC + ORDERING: data progression (raw → analyzed), representative-before-grouped, results-section order = figure order, no-undefined-before-use (figures follow a dependency graph; a panel can only appear once its terms are defined), one fixed representative subject across all representative panels (reliability + anti-cherry-pick), consistent color scheme across the whole paper. The WHY for the scitex-writer figure-first agreement protocol; pairs with `01_figures_01_standards.md` (the rendering layer). Honest-grounding norm from `06_scitexification` carries over to the figure-list integrity rule.
