...
### 1. Figures
- [01_figures_01_standards.md](01_figures_01_standards.md) — Universal scientific-figure RENDERING standards: comparison rules (shared color scale, aligned axes), multi-panel layout, color maps, PDF report layout. Pairs with `figrecipe/21_scientific-figure-patterns.md` for matplotlib code.
- [01_figures_02_logic-and-ordering.md](01_figures_02_logic-and-ordering.md) — Universal scientific-figure LOGIC + ORDERING (9 principles): (1) data progression raw → analyzed, (2) representative-before-grouped, (3) results-section order = figure order, (4) no-undefined-before-use (figures follow a dependency graph; a panel can only appear once its terms are defined), (5) one fixed representative subject across all representative panels (reliability + anti-cherry-pick), (6) consistent color scheme across the whole paper, (7) config-driven figure parameters (colours, representative-subject IDs live in `~/proj/<project>/configs/*.yaml`, not in plot scripts), (8) visual encoding channels — dimensionality of representation (Bertin precedence: position > colour > facet > 3D; "how to represent N elements" enumeration), (9) **no direct numbers outside config — reproducibility hard-rule** — EVERY load-bearing number in the project (epochs, p-thresholds, hyperparameters, results-section numbers) lives in `config/*.yaml`; hardcoded numbers anywhere are forbidden. Strong generalisation of §7 to all numbers. The WHY for the scitex-writer figure-first agreement protocol; pairs with `01_figures_01_standards.md` (the rendering layer). Honest-grounding norm from `06_scitexification` carries over to the figure-list integrity rule.
...
### 7. Paper-writing — reference pointer to scitex-writer
- [07_paper-writing.md](07_paper-writing.md) — Thin reference leaf. The canonical paper-writing FLOW (agent ↔ user agreement protocol, figure-first communication pattern `Fig N → a./b./c.`, per-section flow scaffolds, manuscript-workflow conventions including the `<proj-root>/paper -> .scitex/writer` symlink) lives in scitex-writer's `_skills/scitex-writer/` (leaves 14, 30–36, 40–43). This leaf is a pointer + the boundary statement (`scientific/` carries the tool-agnostic reasoning rules; scitex-writer carries the tool-specific LaTeX/CLI surface). `requires: [scitex-writer]` for declarative loading.
