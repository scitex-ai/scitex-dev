...
- [01_figures_02_logic-and-ordering.md](01_figures_02_logic-and-ordering.md) — Universal scientific-figure LOGIC + ORDERING (8 principles): (1) data progression raw → analyzed, (2) representative-before-grouped, (3) results-section order = figure order, (4) no-undefined-before-use (figures follow a dependency graph; a panel can only appear once its terms are defined), (5) one fixed representative subject across all representative panels (reliability + anti-cherry-pick), (6) consistent color scheme across the whole paper, (7) config-driven figure parameters (colours, representative-subject IDs live in `~/proj/<project>/configs/*.yaml`, not in plot scripts), (8) visual encoding channels — dimensionality of representation (Bertin precedence: position > colour > facet > 3D; "how to represent N elements" enumeration). The WHY for the scitex-writer figure-first agreement protocol; pairs with `01_figures_01_standards.md` (the rendering layer). Honest-grounding norm from `06_scitexification` carries over to the figure-list integrity rule.
