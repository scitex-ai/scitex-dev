...
FastMCP 3.0 removed ``_tool_manager`` and renamed ``get_tools()`` to
``list_tools()`` (returning a list instead of a dict).  This module
provides a unified interface that works with both versions.

Usage::

    from scitex_dev import get_tools_sync
    tools = get_tools_sync(mcp_server)   # {name: Tool}
"""
...
def get_tools_sync(mcp_server, include_mounted: bool = True) -> dict:
    ...
    include_mounted : bool
        If *True* (default), include tools from mounted sub-servers.
    """
