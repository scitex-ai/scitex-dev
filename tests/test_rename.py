...
class TestExecuteRename:
    ...
    def test_execute_blocks_on_uncommitted(self, tmp_path):
        ...
        with patch("scitex_dev.rename.core.has_uncommitted_changes", return_value=True):
    ...
    def test_execute_force_bypasses_uncommitted_check(self, tmp_path):
        ...
        with patch("scitex_dev.rename.core.has_uncommitted_changes", return_value=True):
            with patch(
                "scitex_dev.rename.core.check_directory_safety", return_value=None
            ):
...
class TestSkipIds:
    ...
    def test_skip_symlink_target(self, tmp_path):
        ...
        with patch("scitex_dev.rename.core.check_directory_safety", return_value=None):
            with patch(
                "scitex_dev.rename.core.has_uncommitted_changes",
                return_value=False,
            ):
                execute_rename(
                    "old_target",
...
class TestIO:
    ...
    def test_write_text_sudo_calls_subprocess(self, tmp_path):
        f = tmp_path / "test.txt"
        with patch("scitex_dev.rename.io._sudo_run") as mock_run:
...
class TestSudoPropagation:
    ...
    def test_preview_with_sudo_does_not_write(self, tmp_path):
        ...
        (tmp_path / "test.py").write_text("old = 1\n")
        with patch("scitex_dev.rename.io._sudo_run") as mock_sudo:
