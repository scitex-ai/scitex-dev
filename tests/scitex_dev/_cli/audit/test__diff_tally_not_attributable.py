...
def test_a_violation_spelled_roll_up_is_a_tally():
    ...
    Reported by scitex-cards 2026-08-10 with an emitter survey: auditors spell
    the tally `error(s)` (_project, _django), `violation(s)` (_api, _skills,
    _ecosystem brand + masking) or `finding(s)` (gui, registry-layout,
    local-state). Only the first matched, so the Python-API roll-up fell
    through to `_unparsed_key` and was keyed by its WHOLE LINE -- count
    included.

    THE CONSEQUENCE IS INVERTED, which is why it earns a test: any PR that
...
def test_a_finding_spelled_roll_up_is_a_tally():
    ...
    level and so has never reached the ERRO fail-open. Covered now rather than
    after someone promotes it to error and inherits the same defect."""
    # Arrange
...
def test_the_tail_anchor_still_guards_the_added_nouns():
    """Adding nouns must not widen what gets swallowed. Both of these carry a
    TAIL after the count, so they are findings rather than roll-ups -- checked
    against the real emitter strings from the same survey."""
    # Arrange
    stdout = (
