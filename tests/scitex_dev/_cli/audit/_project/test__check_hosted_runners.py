...
pinned separately by `test_new_hosted_runner_is_not_escalated_in_a_git_repo`,
which uses a REAL git repo — without that, "no escalation" would pass for the
...
def newly_hosted_git_repo(tmp_path: Path) -> Path:
    """Findings for a job MOVED onto a hosted runner atop a clean `develop`.
    ...
    permitting the status quo and forbidding its correction.
