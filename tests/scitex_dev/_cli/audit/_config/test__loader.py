...
def test_heuristic_hybrid(tmp_path):
    _seed_pip(tmp_path)
    _seed_research(tmp_path)
...
def test_load_falls_through_to_heuristic_when_no_config(tmp_path):
    _seed_pip(tmp_path)
...
def test_load_reads_committed_config(tmp_path):
    _seed_pip(tmp_path)
...
def test_load_override_wins_over_config(tmp_path):
    _seed_pip(tmp_path)
    write_config(tmp_path, project_types=["research"])
...
def test_applies_routes_rp_to_research(tmp_path):
    _seed_pip(tmp_path)
    write_config(tmp_path, project_types=["research"])
    cfg = load_config(tmp_path)
...
def test_write_overwrite_replaces(tmp_path):
    write_config(tmp_path, project_types=["pip"])
