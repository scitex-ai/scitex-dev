...
| 5 | **Lint rules drift from the API they enforce** -- a renamed function in figrecipe leaves the rule pointing at a nonexistent symbol, and the bug only shows up months later | **`scitex-dev lint`** (engine, formerly `scitex-linter`) -- aggregates per-package rule plugins via the `scitex_dev.lint.plugins` entry point. Rules ship in the package whose API they enforce, so rename + rule + test land in one PR. Doc-block linting for `.py` / `.ipynb` / `.md` / `.rst`. `scitex-dev lint sweep` walks every package's README + docs in one shot |
