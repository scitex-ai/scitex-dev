...
#
# Designed to:
#   * Live entirely inside an `srun --overlap --jobid <CI_LEASE_JOBID>`
#     allocation so we get no SLURM queue wait and don't touch a real
#     SLURM reservation. The wrapping `srun --overlap` (from ssh_launch.sh)
#     is what gets us *onto* the compute node; this script then runs on
#     that node.
...
#
# Env contract (set by the caller — typically srun's env passthrough):
...
#
# Logs to $RUNNER_HOME/runner.log; never to stdout (parent srun process
# can be detached).
